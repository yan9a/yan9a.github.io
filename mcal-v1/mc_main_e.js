...
var uis={
	...
	LT:1700,UT:2018,//lower and upper threshold for accurate years
};
//-------------------------------------------------------------------------
...
function initc(){
	...
	if (uis.Lang===undefined) {
		...
		uis.Lang=SelectLang(L);//set a global variable in mc_l.js for translation
	}
  ce.innerHTML=UI();
}
//-------------------------------------------------------------------------
...
function UIContent() {
	...
	//------------------------------------------------------------------------
	js=w2j(uis.y,uis.m,1,uis.Type)//Find julian day number of start of
	//the month according to calendar type
	eml=emLen(uis.y,uis.m,uis.Type);//get the length of the month
	...
	for(i=0;i<eml;i++){
		...
		if ((i>=r)&&(js<=je)) {
			...
			tstr="PriDay";
			if ((E.y==uis.cy) && (E.m==uis.cm) && (E.d==uis.cd)) tstr="PriDayToday";
...
function tHead(MS,ME) {
	...
	<p class='PThMC'>";//table header
			str+=n2s(uis.y)+" "+T(ema[uis.m-1])+T(",")+" " //english year and month
...
function ShowDay(js) {
	...
	try {
		...
		if (hs.f) {
			...
			if(hs.my!=M.my || hs.mm!=M.mm || hs.mp!=M.mp || hs.fd!=M.fd || hs.wd!=M.wd)	{ str+= "<br/>It is inconsistent!";}
			str+="</p>";
		}
	}
	catch(err) {}
